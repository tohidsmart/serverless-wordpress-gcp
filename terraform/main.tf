...
# Standalone Service Account for WordPress Cloud Run
# Created here (not in cloud-run module) to avoid circular dependency with service account key
resource "google_service_account" "wordpress_sa" {
  project      = var.project_id
  account_id   = "${var.name_prefix}-wordpress-sa"
  display_name = "Service Account for ${var.name_prefix}-wordpress"
  description  = "Service account for WordPress Cloud Run service"
}
...
module "wordpress_cloudrun" {
  ...
  service_account_email = google_service_account.wordpress_sa.email
